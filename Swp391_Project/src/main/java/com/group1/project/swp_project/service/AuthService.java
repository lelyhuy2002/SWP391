...
public class AuthService {
  ...
  public Users registerUser(RegisterDto registerDto) {
    ...
    Role userRole =
        roleRepository
            .findByRoleName(DEFAULT_ROLE_NAME)
            .orElseThrow(
                () ->
    ...
    UserStatus userStatus =
        userStatusRepository
            .findByStatusName(DEFAULT_STATUS_NAME)
            .orElseThrow(
                () ->
                    new RuntimeException(
                        "Lỗi: Trạng thái người dùng '"
                            + DEFAULT_STATUS_NAME
                            + "' không tìm thấy."));
  ...
  public void resendVerificationCode(String email) {
    ...
    if (verificationToken == null) {
      ...
      emailService.sendVerificationEmail(
          user.getEmail(), "Mã xác thực tài khoản STI Health", newCodeForNewToken);
  ...
  public void resetPasswordWithCode(ResetPasswordWithCodeRequest request) {
    ...
    if (token == null
        || !token.getToken().equals(request.getCode())
