...
public class BlogService {
  ...
  private BlogSummary convertToSummaryDto(Blog blog) {
    String authorName =
        (blog.getCreatedBy() != null && blog.getCreatedBy().getProfile() != null)
  ...
  public List<BlogSummary> getPublishedBlogSummaries() {
    ...
    return publishedBlogs.stream().map(this::convertToSummaryDto).collect(Collectors.toList());
  }
  ...
  public List<BlogSummary> getPendingBlogs() {
    ...
    return pendingBlogs.stream().map(this::convertToSummaryDto).collect(Collectors.toList());
  }
  ...
  public BlogDetail getBlogByIdDetail(int id) {
    return blogRepository.findById(id).map(this::convertToDetailDto).orElse(null);
  }
  ...
  public List<BlogSummary> filterBlogs(String status, Integer topicId) {
    ...
    return result.stream().map(this::convertToSummaryDto).collect(Collectors.toList());
  }
}
